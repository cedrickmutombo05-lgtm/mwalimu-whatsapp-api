...
// --- RAPPEL DU MATIN (AVEC NETTOYAGE EN TEMPS RÉEL) ---
cron.schedule("0 7 * * *", async () => {
    try {
        ...
        for (const user of res.rows) {
            ...
            // Sécurité anti-pollution : on nettoie le rêve avant l'affichage au cas où
            const reveAffiche = user.reve.replace(/Quels sont|territoires|Sud-Ubangi|Bonjour|Mwalimu|\?|!/gi, "").trim() || "pilier du Congo";
...
// --- RECHERCHE SQL AMÉLIORÉE (INFAILLIBLE) ---
async function consulterBibliotheque(phrase) {
    ...
    for (let mot of mots) {
        if (mot.length < 4 || ["quels", "sont", "dans", "votre"].includes(mot)) continue;
        try {
...
app.post("/webhook", async (req, res) => {
    ...
    try {
        ...
        // --- ENRÔLEMENT (SÉCURISATION & ÉLAGAGE) ---
        if (!user) {
        ...
        if (!user.reve) {
            // Sécurité : Si le texte contient des mots de recherche, on ne l'enregistre PAS comme rêve
            if (text.toLowerCase().includes("territoire") || text.toLowerCase().includes("province")) {
                 return await envoyerWhatsApp(from, "🔵 Attendons un peu ! Quel est d'abord ton **rêve professionnel** avant de passer à la géographie ?");
            }
        ...
        const systemPrompt = `
Tu es Mwalimu EdTech, précepteur d'élite congolais. Ton ton est chaleureux mais rigoureux.
...
2. VILLES VS TERRITOIRES : RÈGLE D'OR. Liste les Villes (Boma, Zongo, Beni, Butembo, Uvira, Baraka, Likasi) SEULEMENT si présentes dans SOURCE_SQL.
3. RIGUEUR : Ne résume jamais. Liste chaque territoire de la source SQL sans exception.
4. TON : Présent et vivant. Parle directement à ${user.nom}.
</DIRECTIVES_PRECEPTEUR>
...
<STRUCTURE_LECON>
🔵 [VÉCU] : Anecdote pédagogique humaine sur la région.
🟡 [SAVOIR] :
   - Chef-lieu : [Nom]
...
`;
        ...
        const completion = await openai.chat.completions.create({
            ...
            temperature: 0.0 // AUCUNE INVENTION : Rigueur maximale
        });
