...
app.post("/webhook", async (req, res) => {
    ...
    try {
        ...
        // --- CONSOLIDATION DES ACQUIS : CYCLE ENRÔLEMENT ---
        if (!user) {
        ...
        // --- PRÉPARATION DES DONNÉES SANS PERTE ---
        let rawData = "VIDE";
        if (info) {
...
1. SALUTATION : Commence TOUJOURS par saluer ${user.nom}. Alterne : Ebwe (Kikongo), Mbote, Jambo, Moyo, Betu ou Bonjour.
...
`;
        ...
        const completion = await openai.chat.completions.create({
            ...
            temperature: 0.1 // Précision maximale pour éviter l'oubli de données
        });
