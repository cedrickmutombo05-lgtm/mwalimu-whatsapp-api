...
cron.schedule("0 7 * * *", async () => {
    try {
        ...
        for (const user of res.rows) {
            ...
            const reveAffiche = user.reve.replace(/Quels sont|territoires|Haut-Katanga|Sud-Ubangi|Bonjour|Mwalimu|\?|!/gi, "").trim() || "grand leader";
...
// --- RECHERCHE SQL AMÉLIORÉE ET FLEXIBLE ---
async function consulterBibliotheque(phrase) {
    if (!phrase) return null;
    const nettoyer = (str) => str.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
   
    // On remplace les tirets par des espaces pour que "Haut-Katanga" devienne "Haut Katanga"
    let texteNettoye = nettoyer(phrase).replace(/-/g, " ");
    const mots = texteNettoye.replace(/[?.,!]/g, "").split(/\s+/);

    for (let mot of mots) {
        if (mot.length < 4 || ["quels", "sont", "dans", "votre"].includes(mot)) continue;
        try {
            // Recherche par mot-clé (Katanga, Ubangi, Lualaba, etc.)
    ...
    try {
        ...
        if (!user.reve) {
            if (text.toLowerCase().includes("territoire") || text.toLowerCase().includes("province")) {
                 return await envoyerWhatsApp(from, "🔵 Attendons un peu ! Quel est d'abord ton **rêve professionnel** ?");
            }
...
<DIRECTIVES>
1. SOURCE SQL : Tu parles UNIQUEMENT à partir de la SOURCE_SQL fournie.
2. VILLES VS TERRITOIRES : Respecte la RÈGLE D'OR. Sépare les Villes (Zongo, Beni, Butembo, Uvira, Baraka, Likasi) des Territoires.
3. ABSENCE DE DONNÉES : Si SOURCE_SQL est "AUCUNE DONNÉE", dis poliment que tu n'as pas encore cette province en mémoire, mais cite-en une que tu connais.
4. TON : Chaleureux, patriotique et pédagogique.
</DIRECTIVES>
...
`;
        ...
        const completion = await openai.chat.completions.create({
            ...
            temperature: 0.1 // Légère flexibilité pour le style, mais reste rigide sur les faits
        });
