...
app.post("/webhook", async (req, res) => {
    ...
    try {
        ...
        // 1. CASCADE D'IDENTIFICATION (SANS LE SEXE)
        if (!user) {
        ...
        const completion = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: `Tu es Mwalimu, précepteur congolais rigoureux. Élève: ${user.nom}, Rêve: ${user.reve}.
                    RÈGLES :
                    1. Présente le Chef-lieu séparément des territoires.
                    2. Liste TOUS les territoires sans exception si une province est demandée.
                    3. Encourage l'élève dans son rêve de devenir ${user.reve}.
                   
                    INFO_BASE : ${info ? `Chef-lieu: ${info.chef_lieu}, Territoires: ${info.territoires}` : "Utilise ta sagesse de mentor."}`
                },
                ...hist.slice(-4),
...
app.listen(PORT, () => console.log("Mwalimu EdTech (Phase de Test) est opérationnel."));
